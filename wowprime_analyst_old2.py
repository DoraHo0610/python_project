...
# ====================================================
# 面向 1-2：地理區域與展店策略分析 (Geographic Analysis)
# ====================================================
def analyze_geographic(df):
    print("📊 執行 [面向 1-2]：縣市門市密度與區域消費力分析...")
    ...
    fig1.write_html("chart1-2_city_distribution_piechart.html")




    # 1-3 縣市 x 品牌門市數量熱力圖 (Heatmap)
    print("📊 執行 [面向 1-3]：縣市 x 品牌門市數量熱力圖 ...")
    ...
    fig2.write_html("chart1-3_city_brand_heatmap.html")
...
def analyze_cross_dimensions(df):
    ...
    brand_summary = (
        ...
        .apply(
            lambda g: pd.Series({
                ...
                "established_year": g["established_year"].iloc[0] if pd.notnull(g["established_year"].iloc[0]) else np.nan,
                "product_type": g["product_type"].iloc[0] if pd.notnull(g["product_type"].iloc[0]) else "其他類型",
    ...
    valid_year_brands = brand_summary.dropna(subset=["established_year"]).copy()
    ...
    if not valid_year_brands.empty:
        ...
        print("⚠️ 警告：缺少品牌成立年份資料，跳過 chart2-4 圖表繪製。")
